...
while not success:
    ...
    command = start)
    stopButton = tk.Button(top, height=2, width=20, text ="Stop",
    command = stop)
    startButton.pack()
    stopButton.pack()
    ...
    src = imutils.resize(frame, width=600)

    dim_h, dim_w, channels = src.shape
    ...
    if startTest:
        if timeElapsed < TIME_CAP:
            ...
            if x_coords >= dim_w-50:
                negative = True
            elif x_coords < 50:
                negative = False

            cv2.circle(src, (x_coords, 100), 5, (0,0,255), -1)
            if negative:
                x_coords-=5
            else:
                x_coords+=5
